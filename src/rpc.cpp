...
void GetWalletDump(map<uint160,CKeyDump> &mapDump)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapAddressBook)
    {
        for (map<vector<unsigned char>, CPrivKey>::iterator it = pwalletMain->mapKeys.begin(); it != pwalletMain->mapKeys.end(); ++it)
        {
            CKey key;
        ...
        pwalletMain->GetAllReserveKeyHashes(reserveKeys);
        ...
        for (map<uint160, CKeyDump>::iterator it = mapDump.begin(); it != mapDump.end(); ++it)
        {
            ...
            if (pwalletMain->mapAddressBook.count(strAddress))
            {
                ...
                (*it).second.strLabel = pwalletMain->mapAddressBook[strAddress];
            }
        }
    }
}
...
int RetrieveSecret(const uint160 &address, uint256 &privKey)
{
    if (mapPubKeys.count(address))
    {
        ...
        if (pwalletMain->mapKeys.count(pubKey))
        {
            CPrivKey &cp = pwalletMain->mapKeys[pubKey];
            CKey key;
            key.SetPrivKey(cp);
